...
\item[API-NBA] Interface de Programação de Aplicações da NBA
\item[AUC] Área sob a curva
\item[AUC-PR] Área sob a curva Precisão–Recall
\item[AUC-ROC] Área sob a curva ROC
\item[B2B] Back-to-back
\item[BPM] Box Plus/Minus
\item[CV] Coeficiente de variação
\item[ECE] Erro de calibração esperado
\item[EQM] Erro Quadrático Médio
\item[F1] F1-Score
\item[FN] Falso negativo
\item[FP] Falso positivo
\item[FPR] Taxa de falsos positivos
\item[H2H] Head-to-head
\item[IA] Inteligência Artificial
\item[JSON] JavaScript Object Notation
\item[LightGBM] Light Gradient Boosting Machine
\item[Log loss] Perda logística
...
\item[PPV] Valor preditivo positivo
\item[PR] Precisão–Recall
...
\item[TPR] Taxa de verdadeiros positivos
...
Os dados primários foram extraídos por meio de consultas à \textit{API-NBA}, uma interface de programação de aplicações (API) comercialmente disponível. A escolha desta fonte justifica-se pela sua capacidade de fornecer dados estruturados, consistentes e de alta granularidade, atendendo às necessidades específicas do modelo em dois níveis: dados em nível de jogo e dados em nível de jogador. O primeiro consiste em um conjunto contendo o registro de cada partida, detalhando as equipes mandante e visitante, a data do confronto e os placares finais. O segundo consiste em um conjunto granular que contém as estatísticas individuais (\textit{box score}) de cada atleta para cada partida disputada.
 ...
 Para garantir a consistência da coleta de dados via API e \textit{scraping}, foram implementadas estratégias de múltiplas tentativas de requisição, pausas exponenciais em caso de limites atingidos e intervalos regulares entre chamadas, minimizando falhas e respeitando os termos de uso das fontes.

O escopo temporal dos dados abrange as temporadas regulares da \textit{National Basketball Association} a partir de 2021--2022 até a data de execução do processo de coleta (novembro de 2025), garantindo uma base de dados confiável para a análise.
...
\begin{itemize}
    ...
    \item Avaliação do uso controlado de \textit{odds} como variável explicativa, com testes de sensibilidade para evitar sobreajuste ao mercado.
