...
\begin{description}
    \item[Buffer temporal na engenharia de atributos.] Utilizou-se uma função que aplica um \textit{buffer} temporal de 1 dia (24 horas). Ao calcular os atributos para um jogo na data $T$, a função assegura que apenas dados de jogos ocorridos estritamente antes de $T$ (isto é, até $T-1$) sejam consultados, simulando a informação disponível no momento da previsão.
    \item[Embargo na validação cruzada.] Durante a validação cruzada temporal, introduziu-se um período de “embargo” de 1 dia entre o fim do treino e o início da validação. Isso evita que a performance na validação seja influenciada por dados muito próximos (ou do mesmo dia) dos últimos dados usados no treino, como ilustrado na Figura \ref{fig:embargo}.
\end{description}
...
\begin{itemize}
    ...
    \item \texttt{home\_team\_code} / \texttt{away\_team\_code}: Códigos de três letras identificando as equipes mandante e visitante (por exemplo, 'LAL' para Los Angeles Lakers).
    ...
    \item \texttt{season}: Ano da temporada correspondente (por exemplo, 2021 para a temporada 2021-2022).
    \item \texttt{status}: Status final do jogo (por exemplo, 'Finished').
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{pos}: Posição primária do jogador (por exemplo, 'G' para \textit{Guard}, 'F' para \textit{Forward}, 'C' para \textit{Center}). Pode estar vazia.
    \item \texttt{min}: Minutos jogados na partida (formato texto, por exemplo, '30:15' ou vazio).
...
Os experimentos foram executados em ambiente Python 3.10 com as bibliotecas CatBoost, Optuna, Scikit-learn, Pandas e SHAP. A base histórica compreende jogos e estatísticas individuais de jogadores da NBA a partir de 2021 até 27 de dezembro de 2024 para treino. O conjunto de teste é composto por 783 jogos ocorridos entre 28 de dezembro de 2024 e (confirmar a data certa dps). Todas as rotinas respeitam a cronologia e aplicam embargo temporal de 24 horas, tanto na engenharia de atributos quanto na validação, conforme descrito no Capítulo 3.
...
O modelo final foi treinado no bloco de treino com os hiperparâmetros selecionados:
\begin{itemize}[noitemsep, topsep=0pt]
...
Os resultados do teste cronológico indicam ganho em relação à linha de base no período avaliado. A acurácia ficou 11{,}88 pontos percentuais acima da referência e a AUC-ROC atingiu 0{,}7036, o que aponta capacidade de ordenação entre vitórias e derrotas. Esse desempenho, porém, varia entre equipes e recortes temporais, como detalhado nas análises subsequentes. 

A análise de ablação esclarece a fonte desse ganho. A retirada das variáveis em nível de jogador reduz 4{,}44 p.p. de acurácia e 0{,}0426 de AUC, o que mostra que as agregações individuais adicionam informação que não está presente nas médias de equipe e nos controles de calendário. Em termos práticos, o modelo se beneficia de capturar quem, de fato, compõe a rotação e como esse grupo tem performado recentemente.

Na comparação com trabalhos prévios, os números ficam alinhados à acurácia média relatada por \citeonline{horvat2023} e dentro do intervalo de AUC observado por \citeonline{khanmohammadi2024}. Parte das diferenças decorre de escolhas metodológicas, como janela temporal, período avaliado e escopo de variáveis. Também se observa heterogeneidade entre franquias: elencos mais estáveis tendem a gerar previsões mais consistentes, enquanto fases de reconstrução e trocas frequentes ampliam a variância das métricas.

 Em síntese, integrar histórico recente com informações de jogadores, sob validação temporal com embargo, produz ganhos mensuráveis frente a abordagens apoiadas apenas em médias de equipe, sem sacrificar a interpretabilidade operacional das probabilidades.
