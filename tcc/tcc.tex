...
Após o treinamento do classificador, realizou-se uma etapa adicional para avaliar e, quando necessário, corrigir a qualidade das probabilidades geradas. A motivação é simples: um modelo pode ordenar bem os casos entre vitória e derrota e ainda assim apresentar probabilidades sistematicamente desajustadas. Esse desajuste aparece quando, por exemplo, jogos aos quais o modelo atribui 70\% de chance de vitória culminam em vitórias reais em proporção sensivelmente diferente de 70\%.

O procedimento adotado seguiu três passos. Primeiro, foi medida a confiabilidade das previsões por faixas de probabilidade, juntamente com o cálculo do \textit{Brier score} e a inspeção da curva de calibração. Segundo, foi verificada a presença de viés sistemático de confiança, tanto em direção a favoritos como em direção a azarões. Terceiro, quando esse viés excedeu um limiar pré-definido, aplicou-se uma calibração isotônica como etapa pós-treino para readequar as probabilidades. A calibração utilizou partições temporais consistentes com a validação para evitar o uso indevido de informações futuras.

Essa etapa metodológica tem dois benefícios práticos. Melhora a interpretabilidade das probabilidades, que passam a refletir frequências observadas de forma mais fiel. E favorece decisões dependentes de risco, como a escolha de limiares distintos de 0{,}50 para maximizar métricas específicas.
...
O ganho de 11,88 pontos percentuais em acurácia sobre a linha de base indica avanço significativo. A AUC-ROC de 0,7036 confirma boa discriminação entre vitórias e derrotas. Após calibração isotônica, observou-se redução do desvio médio entre probabilidade prevista e frequência observada.
...
A estabilidade observada não deve ser interpretada como falta de capacidade preditiva, mas como indício de que o modelo prioriza consistência temporal em detrimento da sensibilidade contextual. Em estudos futuros, recomenda-se testar estratégias de \textit{online learning} ou reamostragem incremental, permitindo adaptação contínua às mudanças de elenco e estilo de jogo.
...
\begin{figure}[H]
    \centering
    \caption{Acurácia mensal do modelo no conjunto de teste (temporada 2024–2025)}
    \includegraphics[width=0.85\textwidth]{figuras/acc_geral.jpeg}
    \caption*{
        \small
        Evolução da acurácia mensal do modelo entre outubro e abril da temporada 2024–2025. 
        Observa-se estabilidade em torno de 65\% a 68\%, com leve elevação em março, possivelmente associada à consolidação das rotações e à redução de variabilidade entre equipes. 
        \textit{Fonte:} Elaboração própria (2025).
    }
    \label{fig:acc_mes}
\end{figure}
...
\label{subsec:limiar}
A variação do limiar de decisão permite ajustar precisão e recall conforme objetivo do usuário. Observou-se ponto de equilíbrio adequado em torno de um limiar superior a 0,50. A Tabela \ref{tab:threshold} apresenta um exemplo ilustrativo. Recomenda-se incluir o gráfico da aba \textit{Threshold\_Analysis} como Figura \ref{fig:threshold_curve}.
...
\includegraphics[width=0.75\textwidth]{figuras/threshold_curve.png}
\caption*{\footnotesize Fonte: elaboração própria a partir de \textit{Threshold\_Analysis}.}
\label{fig:threshold_curve}
\end{figure}

\section{Análise de ablação}
\label{sec:ablacao}
Para mensurar a contribuição de grupos de variáveis, foram comparadas três configurações: modelo completo, modelo sem atributos de jogadores e baseline com atributos básicos de equipe e calendário. A Tabela \ref{tab:ablacao} resume os resultados.
...
\begin{tabular}{lccc}
\hline
Configuração & AUC & Acurácia & Diferença vs. completo \\
\hline
Completo & 0,7036 & 65,64\% & — \\
Sem atributos de jogadores & 0,6610 & 61,20\% & −4,44 p.p. \\
Baseline equipe e calendário & 0,5890 & 55,12\% & −10,52 p.p. \\
\hline
\end{tabular}
\end{table}

Os resultados indicam que os atributos em nível de elenco respondem por parcela relevante do ganho preditivo em relação ao baseline.

\section{Explicabilidade das predições}
\label{sec:shap_resultados}
A análise SHAP em teste temporal apontou maior impacto para diferenças de desempenho recente entre mandante e visitante, vantagem de descanso, indicadores de sequência e agregados de jogadores com maior tempo em quadra. Recomenda-se incluir:
\begin{itemize}
\item Gráfico de importância média de atributos em teste como Figura \ref{fig:shap_importance}.
\item Gráfico de valores SHAP por amostra com coloração por feature value como Figura \ref{fig:shap_beeswarm}.
\end{itemize}

\begin{figure}[H]
\centering
\caption{Importância média de atributos segundo SHAP}
\includegraphics[width=0.80\textwidth]{figuras/shap_importances.png}
\caption*{\footnotesize Fonte: elaboração própria a partir de \textit{Feature\_Importances} e \textit{Per\_Game\_Group\_SHAP}.}
\label{fig:shap_importance}
\end{figure}

\begin{figure}[H]
\centering
\caption{Distribuição de valores SHAP em teste}
\includegraphics[width=0.80\textwidth]{figuras/shap_beeswarm.png}
\caption*{\footnotesize Fonte: elaboração própria a partir de \textit{Per\_Game\_SHAP}.}
...
\begin{figure}[H]
\centering
\caption{AUC em função do tamanho da janela de jogos}
\includegraphics[width=0.70\textwidth]{figuras/otw_auc.png}
\caption*{\footnotesize Fonte: elaboração própria a partir de \textit{otw\_analysis\_results\_fixed.xlsx}.}
\label{fig:otw_auc}
\end{figure}

\section{Estudo de caso de predição individual}
\label{sec:pred_individual}
Para um confronto prospectivo, o pipeline gera atributos com base apenas nas informações disponíveis até a véspera e retorna a probabilidade prevista de vitória. Recomenda-se incluir um exemplo de predição com explicabilidade local, conforme arquivo \textit{predicao\_jogo\_individual.xlsx}, destacando os principais fatores contribuintes segundo SHAP local. A Figura \ref{fig:shap_local} pode apresentar a decomposição contribucional da probabilidade para o jogo escolhido.

\begin{figure}[H]
\centering
\caption{Explicação local por SHAP para um jogo específico}
\includegraphics[width=0.80\textwidth]{figuras/shap_local.png}
\caption*{\footnotesize Fonte: elaboração própria a partir de \textit{predicao\_jogo\_individual.xlsx}.}
